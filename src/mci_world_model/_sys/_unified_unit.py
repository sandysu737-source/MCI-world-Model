...
def run_tests():
    ...
    """
    logger.info("=" * 60)
    logger.info("UnifiedInfoUnit Test Suite")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info(f"  PASS: stem={unit.temporal_stem}, branch={unit.temporal_branch}, cyclic={unit.cyclic_code}")
        tests_passed += 1
    ...
    try:
        ...
        logger.info(f"  PASS: direction={unit.direction}, colors={unit.colors}, organs={unit.organs}")
        tests_passed += 1
    except Exception as e:
        logger.warning("异常降级: %s", e, exc_info=True)
        logger.error(f"  FAIL: {e}")
    ...
    try:
        ...
        logger.info(f"  PASS: heaven={heaven}, earth={earth}, human={human}")
        tests_passed += 1
    except Exception as e:
        logger.warning("异常降级: %s", e, exc_info=True)
        logger.error(f"  FAIL: {e}")
    ...
    try:
        ...
        logger.info(f"  PASS: Original ID={original.id[:8]}, Restored ID={restored.id[:8]}")
        tests_passed += 1
    except Exception as e:
        logger.warning("异常降级: %s", e, exc_info=True)
        logger.error(f"  FAIL: {e}")
    ...
    try:
        ...
        logger.info(f"  PASS: trigram={unit.trigram}, hexagram={unit.hexagram_index}")
        tests_passed += 1
    except Exception as e:
        logger.warning("异常降级: %s", e, exc_info=True)
        logger.error(f"  FAIL: {e}")
    ...
    try:
        ...
        logger.info(f"  PASS: random stem={unit.temporal_stem}, branch={unit.temporal_branch}")
        logger.info(f"        hexagram={unit.hexagram_index}, energy={unit.energy_type}")
        tests_passed += 1
    except Exception as e:
        logger.warning("异常降级: %s", e, exc_info=True)
        logger.error(f"  FAIL: {e}")
    ...
    # Summary
    logger.info("\n" + "=" * 60)
    logger.error(f"Test Summary: {tests_passed} passed, {tests_failed} failed")
    logger.info("=" * 60)
