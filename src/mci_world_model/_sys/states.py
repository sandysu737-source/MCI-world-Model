...
class BeliefTracker:
    ...
    def shake(self, memory_id: str, conflict_with: str | None = None) -> BeliefState:
        """
        动摇信念（记忆被反驳/发现冲突）
...
class BayesianBeliefTracker:
    ...
    def shake(self, memory_id: str, conflict_with: str | None = None) -> "BayesianBeliefState":
        """
        动摇信念 — 使用贝叶斯后验更新
    ...
    def _fallback_shake(self, memory_id: str, conflict_with: str | None = None):
        if memory_id not in self._transitions:
            self._fallback_initialize(memory_id)
        return BayesianBeliefState(
            memory_id=memory_id,
    ...
    def _build_state(
        self, belief: "BayesianBelief", memory_id: str, trigger: str | None = None
    ) -> "BayesianBeliefState":
