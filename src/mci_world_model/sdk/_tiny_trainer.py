...
"""MCI World Model v4.4.0 — TinyTrainer. TinyTransformer 训练器。"""
...
try:
    ...
    TinyTransformer = None; TinyTransformerConfig = None
...
class TinyTrainMetrics:
    ...
    def to_dict(self) -> dict:
        return {"epoch":self.epoch,"train_loss":round(self.train_loss,6),"val_loss":round(self.val_loss,6),
                "val_accuracy":round(self.val_accuracy,4),"lr":self.lr,"elapsed_seconds":round(self.elapsed_seconds,1)}

class TinyTrainer:
    def __init__(self, model_config: TinyTransformerConfig | None = None, train_config: TinyTrainConfig | None = None) -> None:
        if TinyTransformer is None: raise ImportError("TinyTransformer not available")
        ...
        self._history: list[TinyTrainMetrics] = []; self._qa_pairs: list[tuple[str, str]] = []
    ...
    def load_qa_data(self, path: str) -> int:
        ...
        logger.info("Loaded %d QA pairs", len(self._qa_pairs))
        return len(self._qa_pairs)

    def train(self) -> list[TinyTrainMetrics]:
        if not self._qa_pairs: logger.warning("No QA data"); return []
        n_val = max(1, int(len(self._qa_pairs)*self.train_cfg.val_split))
        idx = self._rng.permutation(len(self._qa_pairs))
        tr, va = idx[:-n_val], idx[-n_val:]; best_v = float("inf"); pat = 0
        for ep in range(self.train_cfg.n_epochs):
            ...
            logger.info("E%d: tr=%.4f val=%.4f acc=%.1f%%", ep+1, tl, vl, va_acc*100)
            if vl < best_v - self.train_cfg.early_stop_min_delta: best_v = vl; pat = 0
            else: pat += 1
            if pat >= self.train_cfg.early_stop_patience: logger.info("Early stop@%d", ep+1); break
        self.model._trained = True; return self._history

    def _train_epoch(self, indices: np.ndarray) -> float:
        ...
        for s in range(0, len(perm), cfg.batch_size):
            ...
            for i in bi:
                ...
                qe = self.model._forward_single(self.model._tokenize(q))
                ae = self.model._forward_single(self.model._tokenize(a))
                cs = np.dot(qe, ae)/(max(np.linalg.norm(qe),1e-10)*max(np.linalg.norm(ae),1e-10))
                bl += float(-cs+1.0)
            for p in ap: p -= cfg.lr*self.model._fd(p, 1e-6, self._rng)
            total += bl/len(bi); nb += 1
        return total/max(nb,1)

    def _validate(self, indices: np.ndarray) -> float:
    ...
    def _qa_acc(self, indices: np.ndarray) -> float:
        n = min(30, len(indices)); correct = 0
        ae = {}
        for i in indices[:n]:
            _, a = self._qa_pairs[i]
            ae[i] = self.model._forward_single(self.model._tokenize(a))
        for i in range(n):
            q, _ = self._qa_pairs[indices[i]]
            qe = self.model._forward_single(self.model._tokenize(q))
            best = -2.0; bj = -1
            for j in indices[:n]:
    ...
    def save_checkpoint(self, path: str) -> bool:
        try:
            ...
            st = {"version":"4.4.0","model_config":{"d_model":self.model_config.d_model},"history":[m.to_dict() for m in self._history],"n_qa":len(self._qa_pairs)}
    ...
    def __repr__(self) -> str:
        return f"TinyTrainer(d={self.model_config.d_model}, L={self.model_config.n_layers}, qa={len(self._qa_pairs)})"
