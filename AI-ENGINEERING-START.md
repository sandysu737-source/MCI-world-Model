...
### 前端 (Node)
```bash
cd web || cd frontend     # 若前端在子目录
npm run lint              # eslint
npm run test:run          # vitest（或 npm test）
```
...
同一模式重复出现 3 次以上 → 复制 `.ai-templates/`（或 `.ai-skills/` 任意一个）结构，沉淀为五件套：`SKILL.md` + `template.code` + `forbid.list` + `test-case.list` + `error-fix.md`，纳入团队基线。
...
```
（注意：会覆盖 `.ai-rules.md`，项目内若有定制请先备份。）
