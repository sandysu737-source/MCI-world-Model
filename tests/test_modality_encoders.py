...
class TestVisionEncoder:
    ...
    def test_output_dim(self, vision_enc):
        ...
        assert vec.shape == (vision_enc.output_dim,)
    ...
    def test_different_inputs_different_outputs(self, vision_enc):
        """P0-F5 KPI: 不同输入产生不同向量 (L2 > 0.1)。"""
        f1 = np.zeros((32, 32, 3))
        f2 = np.ones((32, 32, 3))
        v1 = vision_enc.encode(f1)
        v2 = vision_enc.encode(f2)
        assert not np.allclose(v1, v2)
        assert np.linalg.norm(v1 - v2) > 0.1

    def test_feature_dim_property(self, vision_enc):
        assert vision_enc.feature_dim == 32

    def test_output_dim_property(self, vision_enc):
        """P0-F5: output_dim = learnable_dim。"""
        assert vision_enc.output_dim == 128

    def test_n_params_positive(self, vision_enc):
        """P0-F5 KPI: 编码器有可学习参数 (>0)。"""
        assert vision_enc.n_params > 0
    ...
    def test_global_stats(self, vision_enc):
        """全局统计字段正确 — 通过 _extract_stat_features 验证。"""
        frame = np.full((16, 16), 0.5)
        stat = vision_enc._extract_stat_features(frame)
        assert stat[0] == pytest.approx(0.5, abs=1e-10)  # mean
        assert stat[1] == pytest.approx(0.0, abs=1e-10)  # std
...
class TestAudioEncoder:
    ...
    def test_output_dim(self, audio_enc):
        ...
        assert vec.shape == (audio_enc.output_dim,)

    def test_silence(self, audio_enc):
        """静音 → RMS ≈ 0 — 通过 _extract_stat_features 验证。"""
        silence = np.zeros(8000)
        stat = audio_enc._extract_stat_features(silence)
        assert stat[0] == pytest.approx(0.0, abs=1e-10)  # RMS
    ...
    def test_sine_wave(self, audio_enc):
        ...
        vec = audio_enc.encode(wave)
        assert vec.shape == (audio_enc.output_dim,)
        stat = audio_enc._extract_stat_features(wave)
        assert stat[0] > 0  # RMS > 0
    ...
    def test_n_params_positive(self, audio_enc):
        """P0-F5 KPI: 编码器有可学习参数 (>0)。"""
        assert audio_enc.n_params > 0
...
class TestThermalEncoder:
    ...
    def test_output_dim(self, thermal_enc):
        ...
        assert vec.shape == (thermal_enc.output_dim,)

    def test_uniform_temp(self, thermal_enc):
        """均匀温度 → std=0 — 通过 _extract_stat_features 验证。"""
        frame = np.full((16, 16), 37.0)
        stat = thermal_enc._extract_stat_features(frame)
        assert stat[0] == pytest.approx(37.0, abs=1e-10)  # mean
        assert stat[1] == pytest.approx(0.0, abs=1e-10)  # std

    def test_hotspot(self, thermal_enc):
        """热点检测 — 通过 _extract_stat_features 验证。"""
        frame = np.full((16, 16), 25.0)
        frame[8, 8] = 80.0  # 热点
        stat = thermal_enc._extract_stat_features(frame)
        assert stat[2] == pytest.approx(80.0, abs=1e-10)  # max
        assert stat[4] == pytest.approx(8.0 / 15, abs=1e-2)  # hot_y_norm
        assert stat[5] == pytest.approx(8.0 / 15, abs=1e-2)  # hot_x_norm
    ...
    def test_n_params_positive(self, thermal_enc):
        """P0-F5 KPI: 编码器有可学习参数 (>0)。"""
        assert thermal_enc.n_params > 0
